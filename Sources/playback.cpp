...
void init_playback(snd_pcm_t **handle, unsigned int *play_freq, snd_pcm_uframes_t *play_period_size,
    ...
    std::cerr << "播放参数: " << "播放频率: " << *play_freq << " 周期大小: "
              << *play_period_size << " 最小播放周期大小: " << play_min_period_size_to_set <<
              " 缓冲区大小: " << *play_buffer_size << std::endl;
    ...
    if (*play_period_size != PLAY_FRAMES_PER_PERIOD) {
        std::cout << "每个周期的播放帧数 ( " << *play_period_size << " ) 与配置 ( "
                  << PLAY_FRAMES_PER_PERIOD << " ) 不符" << std::endl;
        exit(1);
    } else if (*play_buffer_size != PLAY_FRAMES_PER_PERIOD * PLAY_PERIODS_PER_BUFFER) {
        std::cout << "每个设备缓冲区的播放帧数 ( " << *play_buffer_size
                  << " ) 与配置 ( " << PLAY_FRAMES_PER_PERIOD * PLAY_PERIODS_PER_BUFFER << " ) 不符"
                  << std::endl;
        exit(1);
    }
