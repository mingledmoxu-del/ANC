...
void secondary_path_identification(fixed_sample_type           *samples_buffer,
                                   fixed_sample_type           *ref_for_processing,
...
void processing_feedforward_anc(fixed_sample_type *samples_buffer, long unsigned int buffer_length) {
    ...
    static FxLMSFilter<FX_FILTER_LENGTH, FILTER_LENGTH> fxlms_filter(LMS_STEP_SIZE, FX_FILTER_COEFFS);
...
void processing_feedforward_anc_sec_path_modelling(fixed_sample_type *samples_buffer, long unsigned int buffer_length) {
    ...
    for (unsigned long i = 1; i < buffer_length; i += 2) {
        // 添加 DC_REDUCTION_VALUE 以修正误差中的直流偏移，直流偏移会导致不稳定性
...
void processing_feedback_anc(fixed_sample_type *samples_buffer, long unsigned int buffer_length) {
    ...
    static FxLMSFilter<FX_FILTER_LENGTH, FILTER_LENGTH> fxlms_filter_right_channel(LMS_STEP_SIZE, FX_FILTER_COEFFS);
    static FxLMSFilter<FX_FILTER_LENGTH, FILTER_LENGTH> fxlms_filter_left_channel(LMS_STEP_SIZE, FX_FILTER_COEFFS);
    ...
    for (unsigned long i = 1; i < buffer_length; i += 2) {
        sample_type error_sample_right_channel = signed_fixed_to_floating(samples_buffer[i]) + DC_REDUCTION_VALUE;
        sample_type error_sample_left_channel  = signed_fixed_to_floating(samples_buffer[i - 1]) + DC_REDUCTION_VALUE;
        sample_type reference_sample_right_channel =
            error_sample_right_channel +
            sec_path_filter_right_channel.fir_step(signed_fixed_to_floating(output_samples[i]));
        sample_type reference_sample_left_channel =
            error_sample_left_channel +
            sec_path_filter_left_channel.fir_step(signed_fixed_to_floating(output_samples[i - 1]));
        sample_type correction_sample_right_channel =
            fxlms_filter_right_channel.lms_step(reference_sample_right_channel, error_sample_right_channel);
        sample_type correction_sample_left_channel =
            fxlms_filter_left_channel.lms_step(reference_sample_left_channel, error_sample_left_channel);
        ...
        fixed_sample_type fixed_correction_sample_right_channel =
            floating_to_signed_fixed(correction_sample_right_channel * OUTPUT_GAIN);
        fixed_sample_type fixed_correction_sample_left_channel =
            floating_to_signed_fixed(correction_sample_left_channel * OUTPUT_GAIN);
        samples_buffer[i]     = fixed_correction_sample_right_channel;
        samples_buffer[i - 1] = fixed_correction_sample_left_channel;
    }
}

void processing_feedback_anc_sec_path_modelling(fixed_sample_type *samples_buffer, long unsigned int buffer_length) {
    ...
    // 在线次级路径建模静态变量
    static LMSFilter<FX_FILTER_LENGTH> sec_path_lms_estimate_right_channel(SEC_PATH_LMS_STEP_SIZE, FX_FILTER_COEFFS);
    static std::vector<sample_type>    correction_samples_buffer_right_channel(buffer_length / 2, 0.0f);
    static sample_type                 sec_path_estimation_output_right_channel = 0.0f;

    static LMSFilter<FX_FILTER_LENGTH> sec_path_lms_estimate_left_channel(SEC_PATH_LMS_STEP_SIZE, FX_FILTER_COEFFS);
    ...
    for (unsigned long i = 1; i < buffer_length; i += 2) {
        ...
        sec_path_filter_left_channel.set_coefficients(sec_path_lms_estimate_left_channel.fir_filter.get_coefficients());
    }
}
